...
public class RecommendationController {
	...
	public ResponseEntity<String> create(@RequestBody(required = true)String request) 
		...
		Recommendation rec = new Recommendation(h.targetUser, 
				h.movie, 
				(h.loggedInUser == null? null : h.loggedInUser.getEmail()), 
				h.recMessage);
		
		// Save the recommendation
		this.recommendationRepository.saveAndFlush(rec);
		...
		return ResponseEntity.ok().body(new JSONObject()
				...
				.put(JsonStrings.REC_ID, rec.getId())
				.put(JsonStrings.TARGET_EMAIL, rec.getOwner().getEmail())
				.toString());
	}
	...
	public ResponseEntity<String> get(@RequestBody(required = true) String request) 
		...
		for (Recommendation r : h.loggedInUser.getMyReceivedRecs()) {
			recs.add(r);
		}
